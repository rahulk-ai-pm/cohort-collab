...
export default function DiscussionsPage() {
  ...
  const deleteDiscussion = async (discId) => {
    ...
    try {
      await axios.delete(`${API}/discussions/${discId}`, { withCredentials: true });
      setDiscussions(prev => prev.filter(d => d.discussion_id !== discId));
      toast.success('Discussion deleted');
    } catch (err) { toast.error(err.response?.data?.detail || 'Delete failed'); }
  };

  const canModify = (d) => d.author_id === user?.user_id || user?.role === 'admin';
              ...
              <div className="flex items-start justify-between">
                <Link to={`/discussions/${d.discussion_id}`} className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-4 mt-3 text-xs text-slate-400">
                    ...
                    {d.edited_at && <span className="text-slate-300">(edited)</span>}
                  </div>
                </Link>
                {canModify(d) && (
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <button className="p-1.5 hover:bg-slate-100 rounded-lg text-slate-400 hover:text-slate-600 transition-colors shrink-0 ml-2" data-testid={`disc-menu-${d.discussion_id}`}>
